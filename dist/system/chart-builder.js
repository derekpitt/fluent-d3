System.register(['./util/empty-element', 'd3'], function (_export) {
    ...
    return {
        setters: [function (_utilEmptyElement) {
            empty = _utilEmptyElement['default'];
        }, function (_d3) {
            d3 = _d3['default'];
        }],
        execute: function () {
            ChartBuilder = (function () {
                ...
                _createClass(ChartBuilder, [{
                    ...
                    value: function startDraw(where) {
                        empty(where);
                        this._svg = d3.select(where).append("svg").attr("width", this.size.width).attr("height", this.size.height).append("g").attr("transform", 'translate(' + this.padding.left + ', ' + this.padding.top + ')');
                        var _size = this.size;
            ...
            _export('ChartBuilder', ChartBuilder);
        }
    };
});